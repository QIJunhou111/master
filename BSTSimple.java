...
public class BSTSimple<Key extends Comparable<Key>, Value> implements BSTdetail<Key, Value> {
   ...
   //The height of the binary search tree
    ...
    public int height(Node node) {
     ...
     int left =height(node.smaller);
     int right=height(node.larger);
    ...
    public static void main(String[] args) {
     ...
     for(int h=100;h<=10000;h+=100) {
    		for(int i=0; i<h;i++) {
    			String key=String.valueOf(ran1.nextInt(2*h));
    			int val= ran1.nextInt(10000);
    			map.put(key, val);
    		}
    		bst.putAll(map);
    
    		for(int i=0;i<1000*h;i++) {
    			String key =String.valueOf(ran2.nextInt(2*h));
    			int val =ran2.nextInt(10000);
    			if(ran3.nextBoolean())
    				bst.delete(key);
    			else bst.put(key, val);
    		}
